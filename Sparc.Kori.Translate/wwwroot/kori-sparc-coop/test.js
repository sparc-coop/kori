...
const dbName = `joseph-${window.location.host.toLowerCase().replace(/[^a-zA-Z0-9]/g, '_')}`;
...
async function saveTextNodesAsync(nodes) {
    console.log('saveTextNodesAsync', nodes.length, 'nodes found');
    for (let n of nodes) {
...
async function saveKoriTextContentAsync(node) {
    ...
    const languageId = node.parentElement?.lang || document.documentElement.lang || navigator.language;
    const id = text + ':' + languageId;
    ...
    console.log('Saving text content:', doc); 
...
function setupPageChangeListener() {
    console.log('setupPageChangeListener');
